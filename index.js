...
const ultimasRespostasAuto = new Map();
...
function podeResponderAuto(chave, janelaMs = 90000) {
  const agora = Date.now();
  const anterior = ultimasRespostasAuto.get(chave) || 0;
  if (agora - anterior < janelaMs) return false;
  ultimasRespostasAuto.set(chave, agora);
  return true;
}
...
async function enviarTexto(telefoneOuJid, texto) {
  if (!sock || !conectado) throw new Error('WhatsApp não conectado');
  let jid = String(telefoneOuJid || '').includes('@') ? telefoneOuJid : jidDoNumero(telefoneOuJid);
  if (!String(jid).includes('@lid')) {
    const existe = await sock.onWhatsApp(jid);
    jid = existe?.[0]?.jid || jid;
  }
  const resp = await sock.sendMessage(jid, { text: texto });
  return { jid, resposta: resp };
}
async function acaoCliente(telefone, acao) {
  if (acao === 'pix') {
    await enviarTexto(telefone, perguntaQuantidade());
    await salvarContato({ telefone, status: 'interessado', ultima_mensagem: 'Pix solicitado manualmente', interessado: true });
  }
  if (acao === 'dados') {
    await enviarTexto(telefone, respostaComprovante());
    await salvarContato({ telefone, status: 'aguardando_dados', ultima_mensagem: 'Dados solicitados manualmente', interessado: true });
  }
  if (acao === 'finalizar') {
    await enviarTexto(telefone, agradecimentoFinal());
    await salvarContato({ telefone, status: 'finalizado', ultima_mensagem: 'Venda finalizada manualmente', interessado: true });
  }
}
...
async function iniciarWhatsApp() {
  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'close') { conectado = false; const reason = lastDisconnect?.error?.output?.statusCode; console.log('WhatsApp desconectado:', reason); if (reason !== DisconnectReason.loggedOut) setTimeout(iniciarWhatsApp, 3000); }
  });
  sock.ev.on('messages.upsert', async ({ messages }) => {
    for (const msg of messages || []) {
      try {
        ...
        if (fromMe) {
          if (detectarPdfDoAtendente(msg) && podeResponderAuto(`${telefone}:pdf_final`, 120000)) {
            await enviarTexto(jid, agradecimentoFinal());
        ...
        if (comprovante) {
          ...
          if (podeResponderAuto(`${telefone}:comprovante`, 90000)) await enviarTexto(jid, respostaComprovante());
          continue;
        }
        if (qtd) {
          await salvarContato({ telefone, status: 'aguardando_pagamento', ultima_mensagem: texto, interessado: true, quantidade: qtd });
          if (podeResponderAuto(`${telefone}:qtd:${qtd}`, 90000)) await enviarTexto(jid, respostaQuantidade(qtd));
          continue;
        }
        if (interessado) {
          await salvarContato({ telefone, status: 'interessado', ultima_mensagem: texto, interessado: true });
          if (podeResponderAuto(`${telefone}:interesse`, 90000)) await enviarTexto(jid, perguntaQuantidade());
        } else {
...
app.get('/status', (req, res) => res.json({ online: true, sistema: CONFIG.sistema, versao: CONFIG.versao, conectado, numeroConectado, temQr: Boolean(qrDataUrl), ultimaMensagemRecebida: ultimaMensagem, campanhaProgresso }));
...
app.post('/api/acao-cliente', async (req, res) => { try { await acaoCliente(req.body.telefone, req.body.acao); res.json({ sucesso: true }); } catch (e) { res.status(500).json({ sucesso: false, erro: e.message }); } });
app.post('/api/campanha', async (req, res) => {
  ...
  const alvo = contatos.slice(0, limite);
  ...
  for (const c of alvo) {
    try { const texto = textos[Math.floor(Math.random() * textos.length)] || 'HOJE TEM REINO DA SORTE! 🎟️\n\nBilhete por apenas R$ 2,00.\n\nResponda com a quantidade que deseja comprar.'; await enviarTexto(c.telefone, texto); campanhaProgresso.enviados++; await delay(intervalo); } catch { campanhaProgresso.erros++; }
  }
...
*{box-sizing:border-box}body{margin:0;background:#081427;color:#fff;font-family:Arial,sans-serif}header{background:#172554;padding:24px 16px;text-align:center;border-bottom:4px solid #3b63ff}h1{font-size:36px;margin:0}main{max-width:1180px;margin:auto;padding:18px}.grid{display:grid;grid-template-columns:1fr;gap:16px}.card{background:#151c31;border:1px solid #33415f;border-radius:18px;padding:18px}h2{color:#aac7ff;font-size:25px;margin:0 0 14px}input,textarea,select,button{width:100%;padding:15px;border-radius:13px;border:1px solid #40506e;background:#0a1426;color:#fff;font-size:16px;margin:8px 0}textarea{min-height:120px}button{border:0;background:#4f6bed;font-weight:bold;cursor:pointer}.danger{background:#d12d37}.orange{background:#e96f23}.green{background:#178a54}.box{background:#090e1e;border-radius:12px;padding:14px;margin-top:10px;white-space:pre-wrap}.kpi{display:grid;grid-template-columns:repeat(2,1fr);gap:10px}.item{background:#0a1426;border:1px solid #3a4a68;border-radius:14px;padding:13px;margin-top:10px}.tag{display:inline-block;background:#3d4d89;padding:5px 10px;border-radius:99px;font-size:12px}.ok{color:#9dff91;font-weight:bold}.nav{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin:14px 0}.nav button{background:#22345e}.acoes{display:grid;grid-template-columns:1fr 1fr;gap:8px}.acoes button{font-size:13px;padding:10px}@media(min-width:900px){.grid{grid-template-columns:1fr 1fr}.wide{grid-column:1/3}}
</style></head><body><header><h1>👑 Reino Zap PRO</h1><p>Painel profissional de vendas por WhatsApp V15.2</p></header><main>
...
<section class="card wide" id="camp"><h2>3. Campanha / Oferta</h2><p>Use 2 ou mais textos separados por --- para embaralhar.</p><textarea id="msgCamp">🎟️ HOJE TEM REINO DA SORTE!\n\nBilhete por apenas R$ 2,00.\n\nResponda com a quantidade que deseja comprar.\n---\n🍀 Quer participar do Reino da Sorte hoje?\n\nBilhete R$ 2,00. Responda só com a quantidade.</textarea><input id="limite" value="20" placeholder="Limite"><input id="intervalo" value="8000" placeholder="Intervalo ms"><button class="danger" onclick="campanha()">Enviar campanha</button><div id="retCamp" class="box"></div></section>
...
async function campanha(){let r=await api('/api/campanha',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({mensagem:msgCamp.value,limite:limite.value,intervalo:intervalo.value})});retCamp.innerText=j(r)}
...
app.listen(PORT, () => console.log('Reino Zap PRO V15.2 rodando na porta ' + PORT));
