...
const CONFIG = {
  ...
  versao: '17.6.2 ANTI_DUPLICACAO_OPERACIONAL',
  bilheteValor: 2,
...
async function salvarContato({ telefone, nome = '', status = 'novo', ultima_mensagem = '', interessado = false, quantidade = null, comprovante = false }) {
  const valid = validarTelefoneBR(telefone); const tel = valid.ok ? valid.telefone : limparTelefone(telefone); if (!tel) return;
...
function chaveEnvio(destino, texto) { return `${limparTelefone(destino)}::${String(texto || '').slice(0, 120)}`; }
async function enviarTextoDestino(destino, texto, opcoes = {}) {
  if (!sock || !conectado) throw new Error('WhatsApp não conectado');
  const chave = chaveEnvio(destino, texto);
  const agora = Date.now();
  const janela = Number(opcoes.janelaMs || 3500);
  if (!opcoes.forcar && enviosRecentes.has(chave) && agora - enviosRecentes.get(chave) < janela) {
    return { duplicadoBloqueado: true, jid: destino };
  }
  enviosRecentes.set(chave, agora);
  setTimeout(() => enviosRecentes.delete(chave), Math.max(janela, 10000));
...
async function iniciarWhatsApp() {
  ...
  try {
    ...
    sock = makeWASocket({ version, auth: state, printQRInTerminal: false, logger: pino({ level: 'silent' }), browser: ['Reino Zap PRO', 'Chrome', '17.6'] }); iniciando = false;
...
async function processarMensagem(msg) {
  ...
  ultimaMensagem = { de: jid, telefone: telefoneBruto, mensagem: texto || '[mídia]', tipo: comprovante ? 'comprovante/midia' : 'texto', interessado, quantidade: qtd, comprovante, data: new Date().toISOString() };
...
app.post('/api/contatos', async (req, res) => { const valid = validarTelefoneBR(req.body.telefone); if (!valid.ok) return res.status(400).json({ sucesso: false, erro: valid.erro }); await salvarContato({ telefone: valid.telefone, nome: req.body.nome, status: 'novo' }); res.json({ sucesso: true, telefone: valid.telefone }); });
