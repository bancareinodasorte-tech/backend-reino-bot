...
function limparTelefone(telefone = '') { return String(telefone).replace('@s.whatsapp.net', '').replace('@g.us', '').replace('@lid', '').replace(/\D/g, ''); }
function normalizarTelefoneBR(telefone = '') {
  let n = limparTelefone(telefone);
  if (!n) return '';
  if (n.startsWith('00')) n = n.slice(2);
  while (n.startsWith('0') && n.length > 11 && !n.startsWith('55')) n = n.slice(1);

  if (n.startsWith('55')) {
    let local = n.slice(2);
    while (local.startsWith('0') && local.length > 11) local = local.slice(1);
...
function validarTelefoneBR(telefone = '') {
  ...
  if (!/^55\d{11}$/.test(n)) return { ok: false, telefone: n, erro: 'Telefone inválido. Use DDD + número com 9. Exemplo: 88994943632' };
  const ddd = n.slice(2, 4); const numero = n.slice(4);
  if (Number(ddd) < 11 || Number(ddd) > 99) return { ok: false, telefone: n, erro: 'DDD inválido.' };
  if (numero.length !== 9 || numero[0] !== '9') return { ok: false, telefone: n, erro: 'Número inválido. Use celular com 9 após o DDD. Exemplo: 88994943632' };
...
function formatarTelefoneBR(telefone = '') {
  const n = normalizarTelefoneBR(telefone);
  if (!/^55\d{11}$/.test(n)) return telefone || '';
  return `(${n.slice(2,4)}) ${n.slice(4,9)}-${n.slice(9)}`;
}
function moeda(v) { return Number(v || 0).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }); }
function valorBilhetes(qtd = 0) { return Number(qtd || 0) * CONFIG.bilheteValor; }
function tabelaPix() { return Array.from({ length: CONFIG.pixMaxAutomatico }, (_, i) => ({ quantidade: i + 1, valor: valorBilhetes(i + 1), valor_formatado: moeda(valorBilhetes(i + 1)) })); }
...
function respostaQuantidade(qtd) {
  const total = moeda(valorBilhetes(qtd));
  return `Perfeito! 🎟️
...
Nome do recebedor: ${CONFIG.pixNome}
Valor exato: ${total}

⚠️ Após pagar, envie o comprovante aqui nesta conversa.`;
}
function respostaPixManual(qtd) {
  const total = moeda(valorBilhetes(qtd));
  return `Perfeito! 🎟️

Você solicitou ${qtd} bilhetes.
💰 Valor estimado: ${total}

Para pedidos acima de ${CONFIG.pixMaxAutomatico} bilhetes, o atendimento será manual para conferência e liberação correta.

Aguarde que a equipe da REINO DA SORTE vai finalizar seu pedido por aqui. ✅`;
}
...
async function processarMensagem(msg) {
  ...
  if (qtd) {
    if (qtd > CONFIG.pixMaxAutomatico) {
      await salvarContato({ telefone: telefoneBruto, status: 'aguardando_finalizacao', ultima_mensagem: texto, interessado: true, quantidade: qtd });
      await enviarTextoDestino(jid, respostaPixManual(qtd));
      return;
    }
    await salvarContato({ telefone: telefoneBruto, status: 'aguardando_pagamento', ultima_mensagem: texto, interessado: true, quantidade: qtd });
...
app.post('/api/acao-cliente', async (req, res) => {
  try {
    ...
    if (acao === 'pix') {
      ...
      if (qtd > 0) await enviarTextoDestino(telefone, qtd > CONFIG.pixMaxAutomatico ? respostaPixManual(qtd) : respostaQuantidade(qtd));
