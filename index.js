...
function salvarCampanhaLocal(){var nome=el('nomeCamp').value.trim();if(!nome)return alert('Digite o nome da campanha');var arr=campanhasSalvas();arr.push({nome:nome,texto:el('msgCamp').value,criado:new Date().toLocaleString('pt-BR')});gravarCampanhasSalvas(arr);carregarCampanhasSalvas();el('nomeCamp').value='';alert('Campanha salva.')}
...
function excluirCampanhaLocal(){var s=el('campPronta'),opt=s.options[s.selectedIndex];if(!opt||opt.dataset.idx===undefined)return alert('Selecione uma campanha salva para excluir.');var arr=campanhasSalvas(),idx=Number(opt.dataset.idx);if(!arr[idx])return alert('Campanha não encontrada.');if(!confirm('Excluir campanha "'+arr[idx].nome+'"?'))return;arr.splice(idx,1);gravarCampanhasSalvas(arr);carregarCampanhasSalvas();el('campPronta').value='';alert('Campanha excluída.')}
...
app.listen(PORT, () => console.log('Reino Zap PRO V17.5.5 rodando na porta ' + PORT));
