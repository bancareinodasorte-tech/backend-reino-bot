...
function obterPixTabela(qtd) { return memoria.pixTabela.get(String(Number(qtd || 0))) || null; }
function mensagemPixManual(qtd) {
  return `Pedido recebido ✅\n\nQuantidade: ${qtd} bilhete${qtd > 1 ? 's' : ''}\nValor: ${moeda(valorPorQuantidade(qtd))}\n\n⚠️ Pedido acima de ${CONFIG.limitePixAutomatico} bilhetes.\nAguarde atendimento manual para gerar o pagamento correto.`;
}
...
function respostaQuantidade(qtd) {
  ...
  if (qtd > CONFIG.limitePixAutomatico) return mensagemPixManual(qtd);

  const total = moeda(valorPorQuantidade(qtd));
  const pix = obterPixTabela(qtd);

  if (!pix || !pix.codigo) {
    return `Pedido recebido ✅\n\nQuantidade: ${qtd} bilhete${qtd > 1 ? 's' : ''}\nValor: ${total}\n\n⚠️ PIX automático para essa quantidade ainda não foi cadastrado.\nAguarde atendimento manual para gerar o pagamento correto.`;
  }
...
async function processarMensagem(msg) {
  ...
  if (qtd) {
    await salvarContato({ telefone: telefoneBruto, status: qtd > CONFIG.limitePixAutomatico ? 'aguardando_finalizacao' : 'aguardando_pagamento', ultima_mensagem: texto, interessado: true, quantidade: qtd });
...
app.get('/api/pix-tabela', (req, res) => {
  res.json({ sucesso: true, limite: CONFIG.limitePixAutomatico, itens: Array.from(memoria.pixTabela.values()).sort((a,b)=>a.qtd-b.qtd) });
});
app.post('/api/pix-tabela', (req, res) => {
  try {
    ...
    for (const item of linhas) {
      ...
      if (qtd >= 1 && qtd <= CONFIG.limitePixAutomatico && codigo) {
