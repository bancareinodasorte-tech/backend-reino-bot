...
function obterPixCadastrado(qtd) {
  return pixTabela.get(String(Number(qtd || 0))) || '';
}
...
function respostaQuantidade(qtd) {
  ...
  if (qtd > CONFIG.limitePixAutomatico) {
    return `Pedido recebido ✅\n\nQuantidade: ${qtd} bilhete${qtd > 1 ? 's' : ''}\nValor: ${total}\n\n⚠️ Para pedidos acima de ${CONFIG.limitePixAutomatico} bilhetes, o pagamento será gerado pelo atendimento manual.\nAguarde um atendente.`;
  }

  const pix = obterPixCadastrado(qtd);
  if (!pix) {
    return `Pedido recebido ✅\n\nQuantidade: ${qtd} bilhete${qtd > 1 ? 's' : ''}\nValor: ${total}\n\n⚠️ O PIX dessa quantidade ainda não está cadastrado no sistema.\nAguarde um atendente gerar o pagamento correto.`;
  }
...
async function processarMensagem(msg) {
  ...
  if (qtd) { await salvarContato({ telefone: telefoneBruto, status: qtd > CONFIG.limitePixAutomatico ? 'aguardando_finalizacao' : 'aguardando_pagamento', ultima_mensagem: texto, interessado: true, quantidade: qtd }); await enviarTextoDestino(jid, respostaQuantidade(qtd)); return; }
...
app.post('/api/acao-cliente', async (req, res) => {
  try {
    ...
    if (acao === 'pix') {
      ...
      if (qtd > 0) await enviarTextoDestino(telefone, respostaQuantidade(qtd));
...
app.get('/api/pix-tabela', (req, res) => {
  const itens = Array.from(pixTabela.entries()).map(([qtd, codigo]) => ({ qtd: Number(qtd), codigo })).sort((a,b) => a.qtd - b.qtd);
...
app.post('/api/pix-tabela', (req, res) => {
  try {
    ...
    for (const item of itens) {
      ...
      if (qtd >= 1 && qtd <= CONFIG.limitePixAutomatico && codigo) pixTabela.set(String(qtd), codigo);
    }
    res.json({ sucesso: true, total: pixTabela.size });
  } catch (e) {
    res.status(500).json({ sucesso: false, erro: e.message });
  }
});
...
async function salvarPixTabela(){var texto=el('pixTabelaTexto').value||'';var itens=[];texto.split('\n').forEach(function(l){var p=l.indexOf('=');if(p>0){var qtd=Number(l.slice(0,p).trim());var codigo=l.slice(p+1).trim();if(qtd>=1&&qtd<=50&&codigo)itens.push({qtd:qtd,codigo:codigo});}});var r=await api('/api/pix-tabela',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({itens:itens})});setRet('retPixTabela',r);}
async function carregarPixTabela(){var r=await api('/api/pix-tabela');if(r.sucesso){el('pixTabelaTexto').value=(r.itens||[]).map(function(i){return i.qtd+'='+i.codigo}).join('\n');}setRet('retPixTabela',r);}
...
app.listen(PORT, () => console.log('Reino Zap PRO V17.5.7.2 rodando na porta ' + PORT));
