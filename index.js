...
const CONFIG = {
  ...
  versao: '17.5.7',
  bilheteValor: 2,
  pixChave: '6ac75e3e-eae6-44b2-865d-9b136fba7630',
  pixNome: 'REINO DA SORTE',
  suporteTexto: 'https://wa.me/5588994943632',
  intervaloPadrao: 3000,
  limitePadrao: 20,
  validadeCampanhaMinutos: 60,
  limitePixAutomatico: 50
};
...
const memoria = { contatos: new Map(), mensagens: [], campanhas: [], pixTabela: new Map() };
...
function valorPorQuantidade(qtd) { return Number(qtd || 0) * CONFIG.bilheteValor; }
function pixManualMensagem(qtd) {
  return `Esse pedido precisa de atendimento manual. ✅\n\nQuantidade solicitada: ${qtd} bilhete${qtd > 1 ? 's' : ''}\nValor: ${moeda(valorPorQuantidade(qtd))}\n\nAguarde que um atendente vai gerar o PIX correto para você.`;
}
function obterPixTabela(qtd) {
  const n = Number(qtd || 0);
  if (!Number.isFinite(n) || n <= 0) return null;
  return memoria.pixTabela.get(String(n)) || null;
}
...
function respostaQuantidade(qtd) {
  qtd = Number(qtd || 0);
  if (!Number.isFinite(qtd) || qtd <= 0) return perguntaQuantidade();

  if (qtd > CONFIG.limitePixAutomatico) {
    return pixManualMensagem(qtd);
  }

  const total = moeda(valorPorQuantidade(qtd));
  const pix = obterPixTabela(qtd);

  if (!pix || !String(pix.codigo || '').trim()) {
    return `Pedido recebido ✅\n\nQuantidade: ${qtd} bilhete${qtd > 1 ? 's' : ''}\nValor: ${total}\n\n⚠️ PIX automático para essa quantidade ainda não está cadastrado.\nAguarde atendimento manual para gerar o pagamento correto.`;
  }

  return `Perfeito! 🎟️\n\nVocê escolheu: ${qtd} bilhete${qtd > 1 ? 's' : ''}\n💰 Total: ${total}\n\n📲 PIX com valor fechado:\nCopie e cole no banco:\n${pix.codigo}\n\n⚠️ Após pagar, envie o comprovante aqui.`;
}
...
async function processarMensagem(msg) {
  ...
  if (qtd) {
    const statusPix = qtd > CONFIG.limitePixAutomatico ? 'aguardando_finalizacao' : 'aguardando_pagamento';
    await salvarContato({ telefone: telefoneBruto, status: statusPix, ultima_mensagem: texto, interessado: true, quantidade: qtd });
...
app.post('/api/acao-cliente', async (req, res) => {
  try {
    ...
    if (acao === 'pix') {
      const contato = await obterContato(telefone);
      const qtd = Number(req.body.quantidade || contato?.quantidade || 0);
      if (qtd > 0) {
        await salvarContato({ telefone, status: qtd > CONFIG.limitePixAutomatico ? 'aguardando_finalizacao' : 'aguardando_pagamento', quantidade: qtd, interessado: true });
        await enviarTextoDestino(telefone, respostaQuantidade(qtd));
      } else await enviarTextoDestino(telefone, perguntaQuantidade());
    }
    if (acao === 'dados') await enviarTextoDestino(telefone, respostaComprovante());
    if (acao === 'finalizar') {
      await salvarContato({ telefone, status: 'bloqueado_campanha', ultima_mensagem: '[atendimento encerrado / bloqueado para campanhas]' });
...
app.post('/api/campanha', async (req, res) => {
  ...
  const selecionados = Array.isArray(req.body.selecionados) ? req.body.selecionados.map(limparTelefone).filter(Boolean) : [];
  ...
  if (!selecionados.length) {
    return res.status(400).json({
      sucesso: false,
      erro: 'Modo teste ativo: selecione pelo menos 1 cliente na lista antes de enviar campanha. O envio para todos está bloqueado por segurança.'
    });
  }

  const bloqueados = ['finalizado', 'bloqueado_campanha'];
  let alvo = contatos
    .filter(c => selecionados.includes(limparTelefone(c.telefone)))
    .filter(c => !bloqueados.includes(c.status || 'novo'));

  if (!alvo.length) {
    return res.status(400).json({
      sucesso: false,
      erro: 'Nenhum cliente válido selecionado. Clientes finalizados/bloqueados não recebem campanha.'
    });
  }
  ...
  const registro = {
    id: Date.now(),
    nome: req.body.nome || 'Campanha teste',
    total: alvo.length,
    enviados: 0,
    erros: 0,
    expirados: 0,
    inicio: dataHumana(new Date().toISOString()),
    fim: null,
    status: 'rodando'
  };
  ...
  campanhaProgresso = { total: alvo.length, enviados: 0, erros: 0, expirados: 0, status: 'rodando', inicio: registro.inicio, fim: null };

  res.json({
    sucesso: true,
    mensagem: 'Campanha iniciada em modo teste somente para selecionados',
    total: alvo.length
  });
  ...
  async function executar() {
    ...
    for (const c of alvo) {
      ...
      try {
        ...
        registro.enviados++;
        await salvarContato({ telefone: c.telefone, status: c.status || 'novo', ultima_mensagem: '[campanha teste enviada] ' + mensagem });
    ...
    campanhaProgresso.fim = dataHumana(new Date().toISOString());
...
app.get('/api/pix-tabela', (req, res) => {
  const itens = Array.from(memoria.pixTabela.values()).sort((a,b) => Number(a.qtd) - Number(b.qtd));
  res.json({ sucesso: true, limite: CONFIG.limitePixAutomatico, itens });
});
app.post('/api/pix-tabela', (req, res) => {
  const itens = Array.isArray(req.body.itens) ? req.body.itens : [];
  memoria.pixTabela.clear();
  for (const item of itens) {
    const qtd = Number(item.qtd);
    const codigo = String(item.codigo || '').trim();
    if (!Number.isFinite(qtd) || qtd < 1 || qtd > CONFIG.limitePixAutomatico || !codigo) continue;
    memoria.pixTabela.set(String(qtd), { qtd, valor: valorPorQuantidade(qtd), codigo });
  }
  res.json({ sucesso: true, total: memoria.pixTabela.size, limite: CONFIG.limitePixAutomatico });
});
app.get('/api/campanhas', (req, res) => res.json({ sucesso: true, campanhas: memoria.campanhas, progresso: campanhaProgresso }));

function html() { return `<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Reino Zap PRO</title><style>*{box-sizing:border-box}body{margin:0;background:#081427;color:#fff;font-family:Arial,sans-serif}header{background:#172554;padding:24px 16px;text-align:center;border-bottom:4px solid #3b63ff}h1{font-size:32px;margin:0}main{max-width:1180px;margin:auto;padding:16px}.nav{display:flex;gap:8px;overflow-x:auto;margin:12px 0}.nav button{min-width:max-content;background:#253760}.card{background:#151c31;border:1px solid #33415f;border-radius:18px;padding:16px;margin-bottom:16px}h2{color:#aac7ff;font-size:24px;margin:0 0 14px}input,textarea,select,button{width:100%;padding:14px;border-radius:12px;border:1px solid #40506e;background:#0a1426;color:#fff;font-size:15px;margin:7px 0}textarea{min-height:115px}button{border:0;background:#4f6bed;font-weight:bold;cursor:pointer}.danger{background:#d12d37}.orange{background:#e96f23}.green{background:#22a852}.gray{background:#475569}.box{background:#090e1e;border-radius:12px;padding:13px;margin-top:10px;white-space:pre-wrap;overflow:auto}.kpi{display:grid;grid-template-columns:repeat(2,1fr);gap:9px}.item{background:#0a1426;border:1px solid #3a4a68;border-radius:14px;padding:12px;margin-top:10px}.tag{display:inline-block;background:#3d4d89;padding:5px 10px;border-radius:99px;font-size:12px}.ok{color:#9dff91;font-weight:bold}.warn{color:#ffe17a}.row{display:grid;grid-template-columns:1fr 1fr;gap:8px}.bar{height:13px;background:#071024;border-radius:99px;overflow:hidden;border:1px solid #304160}.fill{height:100%;background:#4f6bed;width:0%}.alerta{border-color:#ffcc00;box-shadow:0 0 0 2px rgba(255,204,0,.15)}.small{font-size:13px;color:#b8c6e6}.code{font-size:30px;text-align:center;letter-spacing:4px;font-weight:bold;color:#9dff91}button:disabled{opacity:.55}@media(min-width:900px){.grid{display:grid;grid-template-columns:1fr 1fr;gap:16px}.wide{grid-column:1/3}}</style></head><body><header><h1>👑 Reino Zap PRO</h1><p>Painel profissional de vendas por WhatsApp V17.5.7</p></header><main><div class="nav"><button type="button" data-scroll="dash">Dashboard</button><button type="button" data-scroll="camp">Campanhas</button><button type="button" data-scroll="pix">Tabela PIX</button><button type="button" data-scroll="clientes">Clientes</button><button type="button" data-scroll="atendimento">Atendimento</button></div><div class="grid"><section class="card" id="dash"><h2>1. WhatsApp</h2><div id="status">Carregando...</div><button type="button" id="btnAtualizar">Atualizar status</button><button type="button" class="gray" id="btnNovoQr">Gerar novo QR / Reconectar</button><input id="telParear" inputmode="tel" placeholder="Seu número WhatsApp. Ex: 88988072177"><button type="button" class="green" id="btnParear">Conectar por código</button><div id="codigoParear" class="box"></div><div id="qr"></div><div class="small">Código pelo número só funciona se o WhatsApp/Baileys liberar pareamento. Se retornar erro, use QR Code.</div></section><section class="card"><h2>2. Envio rápido</h2><input id="telRapido" inputmode="tel" placeholder="DDD + telefone. Ex: 88994943632"><textarea id="msgRapida">Teste Reino Zap ✅</textarea><button type="button" class="orange" id="btnEnvioRapido">Enviar mensagem</button><div id="retRapido" class="box"></div></section><section class="card wide" id="camp"><h2>3. Campanha / Oferta PRO — MODO TESTE</h2><select id="campPronta"><option value="">Selecione campanha pronta</option><option value="🎟️ HOJE TEM REINO DA SORTE!\n\nBilhete por apenas R$ 2,00.\n\nResponda com a quantidade que deseja comprar.">Hoje tem Reino</option><option value="🍀 Bora participar do sorteio de hoje?\n\n🎟️ Bilhete R$ 2,00\nResponda só com a quantidade. Ex: 2, 5 ou 10.">Bora participar</option></select><div class="row"><input id="nomeCamp" placeholder="Nome da campanha"><button type="button" class="green" id="btnSalvarCamp">Salvar campanha</button></div><div class="row"><button type="button" class="orange" id="btnEditarCamp">Editar campanha selecionada</button><button type="button" class="danger" id="btnExcluirCamp">Excluir campanha selecionada</button></div><textarea id="msgCamp" placeholder="Selecione uma campanha pronta ou digite uma nova mensagem."></textarea><div class="row"><input id="limite" value="1" placeholder="Limite travado no modo teste"><input id="intervalo" value="3000" placeholder="Intervalo ms"></div><div class="row"><select id="filtro"><option value="selecionados">Somente selecionados</option></select><input id="buscaCamp" placeholder="Envio para todos bloqueado"></div><input id="validade" value="60" placeholder="Validade em minutos"><button type="button" class="danger" id="btnCampanha">Enviar somente selecionados</button><div class="small">Segurança: campanha para todos está bloqueada. Selecione clientes na lista antes de enviar.</div><div class="box"><b>Progresso da campanha</b><div class="bar"><div id="fill" class="fill"></div></div><div id="prog">Parado</div></div><div id="retCamp" class="box"></div></section><section class="card"><h2>4. Adicionar contato</h2><input id="nome" placeholder="Nome"><input id="telefone" inputmode="tel" placeholder="DDD + telefone. Ex: 88994943632"><select id="statusNovo"><option value="novo">Novo</option><option value="interessado">Interessado</option><option value="aguardando_pagamento">Aguardando pagamento</option><option value="finalizado">Finalizado</option></select><button type="button" id="btnAddContato">Salvar contato</button><div id="retContato" class="box"></div></section><section class="card" id="fluxo"><h2>5. Fluxo automático</h2><div class="box">Cliente responde quantidade → até 50 bilhetes o sistema busca o PIX cadastrado com valor fechado; acima disso chama atendimento manual.\nCliente manda comprovante com texto de pagamento → sistema pede NOME e TELEFONE.\nCliente envia dados → atendimento manual.\nTelefone inválido → sistema pede correção.\nMensagem fora do fluxo → mensagem oficial aprovada.</div></section><section class="card wide" id="pix"><h2>6. Tabela PIX por quantidade</h2><div class="box">Cadastre PIX copia e cola com valor fechado de 1 até 50 bilhetes.\nFormato por linha:\n1=PIX_COPIA_E_COLA_R$2\n2=PIX_COPIA_E_COLA_R$4\n\nAcima de 50 bilhetes o sistema chama atendimento manual.</div><textarea id="pixTabelaTexto" placeholder="1=cole aqui o pix de R$ 2,00&#10;2=cole aqui o pix de R$ 4,00&#10;3=cole aqui o pix de R$ 6,00"></textarea><div class="row"><button type="button" class="green" id="btnSalvarPixTabela">Salvar tabela PIX</button><button type="button" id="btnCarregarPixTabela">Carregar tabela</button></div><div id="retPixTabela" class="box"></div></section><section class="card wide"><h2>7. Última mensagem</h2><div id="ultima" class="box">-</div></section><section class="card wide"><h2>8. Últimas campanhas</h2><div id="historicoCamp"></div></section><section class="card wide" id="clientes"><h2>9. Clientes / Seleção</h2><div class="row"><input id="buscaCliente" placeholder="Buscar nome ou telefone"><select id="statusCliente"><option value="todos">Todos status</option><option value="novo">Novo</option><option value="respondeu">Respondeu</option><option value="interessado">Interessado</option><option value="aguardando_pagamento">Aguardando pagamento</option><option value="aguardando_dados">Aguardando dados</option><option value="aguardando_finalizacao">Aguardando finalização</option><option value="finalizado">Finalizado</option><option value="bloqueado_campanha">Bloqueado campanha</option></select></div><button type="button" id="btnAtualizarClientes">Atualizar / Buscar clientes</button><div class="row"><button type="button" id="btnSelTodos">Selecionar todos visíveis</button><button type="button" id="btnLimparSel">Limpar seleção</button></div><div class="kpi" id="kpis"></div><div id="lista"></div></section><section class="card wide" id="atendimento"><h2>10. Atendimento humanizado</h2><p>O alerta toca somente quando o cliente já enviou comprovante e dados.</p><div id="manual"></div></section></div></main><script>var contatos=[],selecionados=new Set(),ultimoManual=0,travado={};function el(id){return document.getElementById(id)}function j(x){return JSON.stringify(x,null,2)}function setRet(id,x){el(id).innerText=typeof x==='string'?x:j(x)}async function api(u,o){var r=await fetch(u,o);var data=await r.json().catch(function(){return{sucesso:false,erro:'Resposta inválida'}});if(!r.ok&&!data.erro)data.erro='Erro HTTP '+r.status;return data}async function bloquear(botao,fn){if(travado[botao.id])return;travado[botao.id]=true;var old=botao.innerText;botao.disabled=true;botao.innerText='Aguarde...';try{return await fn()}finally{botao.disabled=false;botao.innerText=old;setTimeout(function(){travado[botao.id]=false},900)}}function bip(){try{var a=new AudioContext(),o=a.createOscillator(),g=a.createGain();o.connect(g);g.connect(a.destination);o.frequency.value=880;g.gain.value=.08;o.start();setTimeout(function(){o.stop();a.close()},350)}catch(e){}}function atualizaProgresso(p){p=p||{};var total=p.total||0,enviados=p.enviados||0,pct=total?Math.round(enviados/total*100):0;el('fill').style.width=pct+'%';el('prog').innerText=(p.status||'parado')+' • '+enviados+'/'+total+' • '+pct+'% • erros: '+(p.erros||0)+' • expirados: '+(p.expirados||0)}async function atualizar(){try{var s=await api('/status');el('status').innerHTML=s.conectado?'<p class="ok">WhatsApp conectado ✅<br>'+s.numeroConectado+'</p><small>Auth: '+s.auth+' • Versão '+s.versao+'</small>':'<p class="warn">WhatsApp desconectado</p><small>Clique em Atualizar. Se não aparecer QR, use Gerar novo QR.</small>';el('ultima').innerText=j(s.ultimaMensagemRecebida||'-');var q=await api('/qr');el('qr').innerHTML=q.qr?'<img src="'+q.qr+'" style="width:100%;max-width:300px;background:white;padding:8px">':'Sem QR no momento.';atualizaProgresso(s.campanhaProgresso||{});await carregarCampanhas()}catch(e){el('status').innerHTML='Erro ao atualizar: '+e.message}}async function novoQr(){if(!confirm('Isso apaga a sessão atual e gera novo QR. Continuar?'))return;var r=await api('/api/novo-qr',{method:'POST'});alert(r.mensagem||r.erro);setTimeout(atualizar,3500)}async function parear(){el('codigoParear').innerText='Gerando código...';var r=await api('/api/pairing-code',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({telefone:el('telParear').value})});if(r.sucesso)el('codigoParear').innerHTML='<div>Código para vincular no WhatsApp:</div><div class="code">'+r.codigo+'</div><div>Abra WhatsApp > Dispositivos conectados > Conectar com número.</div>';else el('codigoParear').innerText='Erro: '+r.erro}async function enviarRapido(){var r=await api('/api/enviar',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({telefone:el('telRapido').value,mensagem:el('msgRapida').value})});setRet('retRapido',r)}async function addContato(){var r=await api('/api/contatos',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({nome:el('nome').value,telefone:el('telefone').value,status:el('statusNovo').value})});setRet('retContato',r);await carregarClientes()}function campanhasSalvas(){try{return JSON.parse(localStorage.getItem('reinoCampanhasSalvas')||'[]')}catch(e){return[]}}
function gravarCampanhasSalvas(arr){localStorage.setItem('reinoCampanhasSalvas',JSON.stringify(arr||[]))}
function carregarCampanhasSalvas(){var select=el('campPronta');if(!select)return;var atual=select.value;var fixas=['','🎟️ HOJE TEM REINO DA SORTE!\\n\\nBilhete por apenas R$ 2,00.\\n\\nResponda com a quantidade que deseja comprar.','🍀 Bora participar do sorteio de hoje?\\n\\n🎟️ Bilhete R$ 2,00\\nResponda só com a quantidade. Ex: 2, 5 ou 10.'];while(select.options.length>3)select.remove(3);campanhasSalvas().forEach(function(c,i){var opt=document.createElement('option');opt.value=c.texto;opt.textContent='Salva: '+c.nome;opt.dataset.idx=i;select.appendChild(opt)});if(atual)select.value=atual}
function usarCampanha(){var s=el('campPronta');if(!s.value){el('msgCamp').value='';el('nomeCamp').value='';return;}el('msgCamp').value=s.value.replaceAll('\\n',String.fromCharCode(10));var opt=s.options[s.selectedIndex];if(opt&&opt.dataset.idx!==undefined){var c=campanhasSalvas()[Number(opt.dataset.idx)];if(c)el('nomeCamp').value=c.nome}}
function salvarCampanhaLocal(){var nome=el('nomeCamp').value.trim();if(!nome)return alert('Digite o nome da campanha');var arr=campanhasSalvas();var s=el('campPronta'),opt=s.options[s.selectedIndex];if(opt&&opt.dataset.idx!==undefined){var idx=Number(opt.dataset.idx);if(arr[idx]){arr[idx]={...arr[idx],nome:nome,texto:el('msgCamp').value,editado:new Date().toLocaleString('pt-BR')};gravarCampanhasSalvas(arr);carregarCampanhasSalvas();alert('Campanha atualizada.');return;}}arr.push({nome:nome,texto:el('msgCamp').value,criado:new Date().toLocaleString('pt-BR')});gravarCampanhasSalvas(arr);carregarCampanhasSalvas();el('nomeCamp').value='';alert('Campanha salva.')}
function editarCampanhaLocal(){var s=el('campPronta'),opt=s.options[s.selectedIndex];if(!opt||opt.dataset.idx===undefined)return alert('Selecione uma campanha salva para editar.');var arr=campanhasSalvas(),idx=Number(opt.dataset.idx);if(!arr[idx])return alert('Campanha não encontrada.');var nome=el('nomeCamp').value.trim()||arr[idx].nome;if(!confirm('Salvar alterações nesta campanha?'))return;arr[idx]={...arr[idx],nome:nome,texto:el('msgCamp').value,editado:new Date().toLocaleString('pt-BR')};gravarCampanhasSalvas(arr);carregarCampanhasSalvas();alert('Campanha editada.')}
function excluirCampanhaLocal(){var s=el('campPronta'),opt=s.options[s.selectedIndex];if(!opt||opt.dataset.idx===undefined)return alert('Selecione uma campanha salva para excluir.');var arr=campanhasSalvas(),idx=Number(opt.dataset.idx);if(!arr[idx])return alert('Campanha não encontrada.');var nome=arr[idx].nome;if(!confirm('Excluir campanha: '+nome+' ?'))return;arr.splice(idx,1);gravarCampanhasSalvas(arr);carregarCampanhasSalvas();el('campPronta').selectedIndex=0;el('nomeCamp').value='';alert('Campanha excluída com sucesso.')}
async function campanha(){if(!selecionados.size){setRet('retCamp',{sucesso:false,erro:'Selecione pelo menos 1 cliente. Envio para todos está bloqueado no modo teste.'});return;}if(!el('msgCamp').value.trim()){setRet('retCamp',{sucesso:false,erro:'Selecione uma campanha pronta ou digite a mensagem antes de enviar.'});return;}var r=await api('/api/campanha',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({nome:el('nomeCamp').value||'Campanha teste',mensagem:el('msgCamp').value,limite:el('limite').value,intervalo:el('intervalo').value,filtro:'selecionados',busca:el('buscaCamp').value,validadeMinutos:el('validade').value,selecionados:Array.from(selecionados)})});setRet('retCamp',r);setTimeout(atualizar,1200)}async function carregarCampanhas(){var r=await api('/api/campanhas');var arr=r.campanhas||[];el('historicoCamp').innerHTML=arr.map(function(c){return'<div class="item"><b>'+c.nome+'</b><br>Status: '+c.status+'<br>Enviados: '+c.enviados+'/'+c.total+'<br>Erros: '+c.erros+' • Expirados: '+(c.expirados||0)+'<br>Início: '+(c.inicio||'-')+'</div>'}).join('')||'<div class="item">Nenhuma campanha ainda</div>';atualizaProgresso(r.progresso||{})}async function carregarClientes(){var url='/api/contatos?busca='+encodeURIComponent(el('buscaCliente').value||'')+'&status='+encodeURIComponent(el('statusCliente').value||'todos');var r=await api(url);contatos=r.contatos||[];var sts=['novo','respondeu','interessado','aguardando_pagamento','aguardando_dados','aguardando_finalizacao','finalizado','bloqueado_campanha'];el('kpis').innerHTML=sts.map(function(s){return'<div class="item"><b>'+contatos.filter(function(c){return(c.status||'novo')===s}).length+'</b><br><span class="tag">'+s+'</span></div>'}).join('');el('lista').innerHTML=contatos.map(function(c){return cardCliente(c,false)}).join('')||'<div class="item">Nenhum contato salvo</div>';var man=contatos.filter(function(c){return c.status==='aguardando_finalizacao'});el('manual').innerHTML=man.map(function(c){return cardCliente(c,true)}).join('')||'<div class="item">Nenhum atendimento manual pendente</div>';if(man.length>ultimoManual)bip();ultimoManual=man.length}function cardCliente(c,manual){var tel=c.telefone||'',chk=selecionados.has(tel)?'checked':'',cls=manual?'item alerta':'item';return'<div class="'+cls+'"><label><input style="width:auto" type="checkbox" data-sel="'+tel+'" '+chk+'> <b>'+(c.nome||'Sem nome')+'</b></label><br>'+tel+'<br><span class="tag">'+(c.status||'novo')+'</span><br>Última: '+(c.ultima_mensagem||'-')+'<div class="row"><input id="qtd_'+tel+'" inputmode="numeric" placeholder="Qtd. bilhetes" value="'+(c.quantidade||'')+'"><button type="button" data-acao="pix" data-tel="'+tel+'">Enviar Pix</button></div><div class="row"><button type="button" data-acao="dados" data-tel="'+tel+'">Pedir dados</button><button type="button" class="green" data-acao="finalizar" data-tel="'+tel+'">Encerrar</button></div></div>'}function selecionarVisiveis(){contatos.forEach(function(c){selecionados.add(c.telefone)});carregarClientes()}function limparSelecao(){selecionados.clear();carregarClientes()}async function acaoCliente(tel,acao){var qtdEl=el('qtd_'+tel);var qtd=qtdEl?qtdEl.value:'';var r=await api('/api/acao-cliente',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({telefone:tel,acao:acao,quantidade:qtd})});alert(r.sucesso?'Ação enviada':'Erro: '+r.erro);carregarClientes()}
async function carregarPixTabela(){var r=await api('/api/pix-tabela');if(!r.sucesso){setRet('retPixTabela',r);return;}el('pixTabelaTexto').value=(r.itens||[]).map(function(i){return i.qtd+'='+i.codigo}).join('\n');setRet('retPixTabela',{sucesso:true,total:(r.itens||[]).length,limite:r.limite});}
async function salvarPixTabela(){var linhas=String(el('pixTabelaTexto').value||'').split('\n');var itens=[];linhas.forEach(function(l){var p=l.indexOf('=');if(p>0){var qtd=Number(l.slice(0,p).trim());var codigo=l.slice(p+1).trim();if(qtd>=1&&qtd<=50&&codigo)itens.push({qtd:qtd,codigo:codigo});}});var r=await api('/api/pix-tabela',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({itens:itens})});setRet('retPixTabela',r);}
document.addEventListener('click',function(ev){var t=ev.target;if(t.dataset.scroll)document.getElementById(t.dataset.scroll).scrollIntoView({behavior:'smooth'});if(t.id==='btnAtualizar')bloquear(t,atualizar);if(t.id==='btnNovoQr')bloquear(t,novoQr);if(t.id==='btnParear')bloquear(t,parear);if(t.id==='btnEnvioRapido')bloquear(t,enviarRapido);if(t.id==='btnSalvarCamp')bloquear(t,salvarCampanhaLocal);if(t.id==='btnEditarCamp')bloquear(t,editarCampanhaLocal);if(t.id==='btnExcluirCamp')bloquear(t,excluirCampanhaLocal);if(t.id==='btnCampanha')bloquear(t,campanha);if(t.id==='btnSalvarPixTabela')bloquear(t,salvarPixTabela);if(t.id==='btnCarregarPixTabela')bloquear(t,carregarPixTabela);if(t.id==='btnAddContato')bloquear(t,addContato);if(t.id==='btnAtualizarClientes')bloquear(t,carregarClientes);if(t.id==='btnSelTodos')selecionarVisiveis();if(t.id==='btnLimparSel')limparSelecao();if(t.dataset.acao)bloquear(t,function(){return acaoCliente(t.dataset.tel,t.dataset.acao)})});document.addEventListener('change',function(ev){var t=ev.target;if(t.id==='campPronta')usarCampanha();if(t.dataset.sel){if(t.checked)selecionados.add(t.dataset.sel);else selecionados.delete(t.dataset.sel)}});setInterval(atualizar,15000);carregarCampanhasSalvas();carregarPixTabela();atualizar().then(carregarClientes);</script></body></html>`; }
app.get('/', (req, res) => res.redirect('/painel'));
app.get('/painel', (req, res) => res.send(html()));
app.listen(PORT, () => console.log('Reino Zap PRO V17.5.7 rodando na porta ' + PORT));
